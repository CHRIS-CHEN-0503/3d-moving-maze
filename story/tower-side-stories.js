...
(function (root, factory) {
  ...
  const STORIES = freeze({
    threads: {
      ...
      steps:[
        {index:0,title:'窗邊的線結',prompt:'線頭纏著一片小窗形的木牌。門邊的接線次序若輪到這一段，就鬆開背面的活結，再接上紅線；不要用力拉扯另一端。',actionLabel:'接回窗邊紅線',success:'木牌輕碰樹根，聲音說：「我把那扇窗留著了。」',failure:'這段線還繃著；先按門邊次序接好前一段。'},
        ...
        {index:2,title:'岔路上的線結',prompt:'岔路標記只畫了一張空椅子，椅背的紅線還在微微顫動。照門邊的順序把紅線接好，就能和另一端的人聯絡，不必要求他立刻趕來。',actionLabel:'接回岔路紅線',success:'空椅子的圖樣亮起，線的彼端說：「別急，我還認得回來的路。」',failure:'岔路線還沒接到前段；請按照提示逐段完成。'}
      ],
      outro:[
    ...
    mirrors: {
      ...
      steps:[
        {index:0,title:'晴雨鏡',prompt:'鏡座原文：「前後兩句話都要記下來。」聲音說：「我喜歡雨聲，但不喜歡鞋子淋濕。」哪種紀錄符合原文？',options:['只留下她喜歡雨聲','把兩句原話都記下','刪掉兩句，避免矛盾'],correctChoice:1,actionLabel:'回答晴雨鏡',success:'鏡面第一次同時映出雨傘和陽光。米菈說：「對，兩個都是我。」',failure:'鏡座要求兩句都留下；相反的感受不代表其中一句是假的。'},
        {index:1,title:'留白鏡',prompt:'鏡座原文：「沒有回答時，不可以替對方決定答案。」鏡中人還沒決定是否離開，紀錄應該怎麼寫？',options:['她已同意離開','她已拒絕離開','尚未作答，保留空白'],correctChoice:2,actionLabel:'回答留白鏡',success:'空白不再閃成錯誤訊號，鏡子安靜留下了等待的位置。',failure:'規則說不能代替別人回答；沒有回答，不代表同意或拒絕。'},
        {index:2,title:'名字鏡',prompt:'鏡座原文：「記下原話，也要記下是誰說的。」兩位旅人說了相反的話，要留下怎樣的紀錄？',options:['分別署名，保留兩句','用票數較多的一句代替','把兩句合成大家都同意'],correctChoice:0,actionLabel:'回答名字鏡',success:'兩道倒影各自找到姓名的位置，沒有誰被改寫成另一個人。',failure:'鏡座要求連同說話者保存；不能把不同人的句子合成一個答案。'}
      ],
      outro:[
    ...
    clockwork: {
      ...
      steps:[
        {index:0,title:'緩衝輪',prompt:'小輪齒間塞著一張舊紙條：「門鈴響起後，先等一等。」取下紙屑後，它能讓機芯先停一下，再決定下一步。',actionLabel:'收好緩衝輪',success:'緩衝輪放進工具盒，洛恩認出紙條上的筆跡，說這是師傅留下的修理提醒。',failure:'先把零件收妥，才有辦法修理中央機芯。'},
        ...
        {index:2,title:'第一號機芯',prompt:'機芯留著兩個空槽。必須先收集緩衝輪與回程簧片，才能讓門先等人開啟，並且能從兩邊自由開關。',actionLabel:'裝回兩件零件',success:'木門先響了一下鈴，等你伸手，才緩緩打開。',failure:'缺少零件。先找到緩衝輪與回程簧片，再來修機芯。'}
      ],
      outro:[
    ...
    tribunal: {
      ...
      steps:[
        ...
        {index:2,title:'封存台',prompt:'請先讀完兩份證據。依照兩份頁腳規則，該怎麼保存，才能讓以後讀到的人看見完整資料，自己判斷發生了什麼事？',options:['只留下「奧倫沒有責任」的結論','刪掉錄音，只保留他的道歉','保留錄音和附註，寫清楚來源，讓大家查證'],correctChoice:2,actionLabel:'選擇保存方式',success:'封存台留下兩份來源清楚的原件，判詞欄保持空著。',failure:'兩份規則要求保留原件與來源，附註不能取代原件；請再核對證據。'}
      ],
      outro:[
    ...
    supper: {
      ...
      steps:[
        ...
        {index:1,title:'手套少年的椅子',prompt:'少年說：「我要甜的、可以用手拿著吃的麵包。熱湯會讓我的手套濕掉。」餐台應端哪一份？',options:['蜜果軟麵包（甜、可手持）','熱蘑菇湯（鹹、液體）','暖穀鹹粥（鹹、需用匙）'],correctChoice:0,actionLabel:'選擇少年的晚餐',success:'少年把麵包掰成兩半，分給你一半。「要不要坐下吃一點？休息好了再出發。」',failure:'他要甜麵包、能用手拿，不是湯或鹹粥；請再選一份。'},
        {index:2,title:'抱枕老人的椅子',prompt:'老人說：「清淡、不辣的蔬菜湯就好。我只想喝湯，今晚先不吃甜點。」餐台應端哪一份？',options:['椒火濃湯（辣）','蜜雲布丁（甜點）','晴蔬清湯（清淡、不辣）'],correctChoice:2,actionLabel:'選擇老人的晚餐',success:'老人把枕頭放到空椅上，笑著說，「你也坐下休息吧，那張椅子是留給你的。」',failure:'他要清淡不辣的蔬菜湯，暫時不吃甜點；標示裡就有答案。'}
      ],
      outro:[
