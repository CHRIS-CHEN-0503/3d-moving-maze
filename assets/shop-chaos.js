(function(){
  ...
  let atlas=null;const trapMaps=new Map();
  function trapTexture(kind){
    if(trapMaps.has(kind))return trapMaps.get(kind);
    const texture=new THREE.Texture();texture.colorSpace=THREE.SRGBColorSpace;
    const i=['oil','glue','foam','boost'].indexOf(kind);texture.repeat.set(.5,.5);texture.offset.set(i%2*.5,i<2?.5:0);
    trapMaps.set(kind,texture);
    if(!atlas&&typeof Image!=='undefined'){
      atlas=new Image();atlas.onload=()=>{for(const t of trapMaps.values()){t.image=atlas;t.needsUpdate=true;}for(const t of traps)if(t)t.mesh.visible=!t.used;};
      atlas.src='assets/shop-traps-v1.webp';
    }
    if(atlas?.complete&&atlas.naturalWidth){texture.image=atlas;texture.needsUpdate=true;}
    return texture;
  }
  function area(x,z,id){
    const cells=[{x,z}],size=G.cell,hash=((MP.seed||0)^Math.imul(epoch+1,2654435761)^Math.imul(id+1,2246822519))>>>0,wanted=1+hash%4;
    const directions=[[size,0],[0,size],[-size,0],[0,-size]];
    for(let i=0;i<cells.length&&cells.length<wanted;i++)for(let k=0;k<4&&cells.length<wanted;k++){
      const [dx,dz]=directions[(k+(hash>>>3))%4],p={x:cells[i].x+dx,z:cells[i].z+dz};
      if(Math.abs(p.x)>(G.mazeW-1)*size/2||Math.abs(p.z)>(G.mazeH-1)*size/2||!safe(p.x,p.z)||cells.some(c=>c.x===p.x&&c.z===p.z))continue;
      if(playerInWall(p.x,p.z,.3)||playerInWall(cells[i].x+dx/2,cells[i].z+dz/2,.3))continue;
      if(traps.some(t=>t&&t.cells.some(c=>Math.hypot(c.x-p.x,c.z-p.z)<size*.8))||(G.goods||[]).some(g=>g&&Math.hypot(g.x-p.x,g.z-p.z)<2)||boxes.some(b=>Math.hypot(b.x-p.x,b.z-p.z)<size))continue;
      cells.push(p);
    }
    return cells;
  }
  ...
  function clearMap(){if(root){root.parent?.remove(root);disposeSceneObject(root);}trapMaps.clear();root=null;traps=[];boxes=[];flights=[];}
  ...
  function add(kind,x,z,by='',id=traps.length){
    ...
    const group=new THREE.Group(),cells=area(x,z,id);
    const geometry=new THREE.PlaneGeometry(G.cell,G.cell),material=new THREE.MeshBasicMaterial({map:trapTexture(kind),transparent:true,alphaTest:.1,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-1});
    for(const cell of cells){const image=new THREE.Mesh(geometry,material);image.rotation.x=-Math.PI/2;image.position.set(cell.x-x,.055,cell.z-z);group.add(image);}
    group.userData.trapKind=kind;group.userData.cells=cells;
    group.visible=!atlas||!!atlas.naturalWidth;
    group.position.set(x,0,z);root.add(group);
    traps[id]={id,kind,x,z,cells,by,armed:now()+(by?1500:2500),mesh:group,used:false};return true;
  }
  function addBox(x,z){
  ...
  function rebuild(){
    ...
    for(let i=0,tries=0;i<target&&tries<500;tries++){
      ...
      if(!safe(p.x,p.z)||playerInWall(p.x,p.z,.3)||traps.some(t=>t&&t.cells.some(c=>Math.hypot(c.x-p.x,c.z-p.z)<G.cell))||(G.goods||[]).some(g=>g&&Math.hypot(g.x-p.x,g.z-p.z)<2))continue;
    ...
    for(let tries=0;boxes.length<count&&tries<500;tries++){
      const p=cellToWorld(Math.floor(rand()*G.mazeW),Math.floor(rand()*G.mazeH));
      if(!safe(p.x,p.z)||playerInWall(p.x,p.z,.3)||traps.some(t=>t&&t.cells.some(c=>Math.hypot(c.x-p.x,c.z-p.z)<G.cell))||[...boxes,...(G.goods||[])].some(t=>t&&Math.hypot(t.x-p.x,t.z-p.z)<G.cell))continue;
      addBox(p.x,p.z);
    }
  }
  function start(){
  ...
  function hit(id,t){
    // Never punish a player with an invisible trap while its artwork is unavailable.
    if(atlas&&!atlas.naturalWidth)return;
    const p=botPosOf(id);if(!p||t.used||now()<t.armed||!t.cells.some(c=>Math.hypot(p.x-c.x,p.z-c.z)<G.cell*.38)||now()<(effects[id]?.immune||0))return;
  ...
  window.ShopChaos={start,stop,rebuild,input,speed,tick,collectForBot,reserved:(x,z)=>traps.some(t=>t?.cells.some(c=>Math.hypot(c.x-x,c.z-z)<G.cell))||boxes.some(b=>Math.hypot(b.x-x,b.z-z)<G.cell)};
