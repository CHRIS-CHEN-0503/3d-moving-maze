...
(function(){
  ...
  for(const [key,def] of Object.entries(GameRules.FIELDS)){
    ...
    if(def.choices)for(const size of def.choices){const option=document.createElement('option');option.value=size;option.textContent=key==='teamSize'?size+' 對 '+size:size+' × '+size;input.appendChild(option);}
